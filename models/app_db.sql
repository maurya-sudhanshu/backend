...
CREATE TABLE `subscriptions` (
  ...
  `max_store_owners` int(11) NOT NULL DEFAULT 5,
...
CREATE TABLE `users` (
  ...
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `role` enum('admin','owner','agent','store') NOT NULL DEFAULT 'owner',
...
ALTER TABLE `subscriptions`
  ADD PRIMARY KEY (`id`),
  ADD KEY `subscriptions_user_idx` (`user_id`);
...
ALTER TABLE `users`
  ...
  ADD KEY `users_agent_idx` (`created_by_agent_id`);
...
ALTER TABLE `users`
  ADD CONSTRAINT `users_created_by_agent_id_fk` FOREIGN KEY (`created_by_agent_id`) REFERENCES `users` (`id`) ON DELETE SET NULL ON UPDATE NO ACTION;
COMMIT;
